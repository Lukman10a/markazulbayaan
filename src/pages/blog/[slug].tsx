...
import { getPostData, getAllPostIds, getAllFeaturedPostIds, getFeaturedPostData } from "@/cms/utils";
      ...
      <header className="mb-8 flex items-center justify-center flex-col">
        <div className='w-2/3 md:px-5'>
          <p className="text-sm mb-2">
            <span className="font-bold text-lg text-amber-900">{currentPost.author}</span>
          </p>
          <p className='text-xs mb:5 md:mb-0'>Posted on {formatDate(currentPost.date)}</p>
          <h1 className="xs:text-sm sm:text-lg md:text-4xl text-neutral-800 font-bold mb-7 md:mb-4 md:mx-auto md:py-4">{currentPost.title}</h1>
        </div>
        <div className="relative w-full h-96 rounded-lg overflow-hidden shadow-lg mb-8">
          <Image src={currentPost.thumbnail} alt={currentPost.title} layout="fill" objectFit="cover" />
        </div>
      </header>
